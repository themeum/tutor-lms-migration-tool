    ...
    if (! class_exists('LDtoTutorMigration')) {
        class LDtoTutorMigration
        {
            ...
            public function ld_migrate_course_to_tutor($return_type = false){
                global $wpdb;
                $ld_courses = $wpdb->get_results("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'sfwd-courses' AND post_status = 'publish';");
                ...
                if (tutils()->count($ld_courses)) {
                    ...
                    foreach ($ld_courses as $ld_course) {
                        $course_i++;
                        $post_data = get_post( $ld_course->ID );
                        $course_id = $this->insert_post($course_type, $post_data->post_title, $post_data->post_content, $post_data->post_author, 0, '');
                        if ($course_id) {
                            $this->migrate_course($ld_course->ID, $course_id);
                            update_option('_tutor_migrated_items_count', $course_i);

                            // Attached Product
                            $this->attached_product($course_id, $post_data->post_title);
